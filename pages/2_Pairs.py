...
# Setup page
st.title("Crypto Tracker - Pairs")
st.subheader("View your trade history.")

# Sidebar with use input
st.sidebar.header("Crypto Pair Tracker")
crypto_pair = st.sidebar.text_input("Pair Name")
start_date = st.sidebar.date_input("Start date")
start_date = start_date.strftime("%Y-%m-%d")
get_currency = st.sidebar.button("Get Pair Info")
...
if get_currency:
    trades_out = crypto_tracker.pair_trades(crypto_pair, start_date)
    ...
    st.download_button(
        label="Download Data",
        data=trade_csv,
        file_name=f"{crypto_pair}_trade_data.csv",
        mime="text/csv"
    )

if get_profit:
    trades_out = crypto_tracker.pair_trades(crypto_pair, start_date).reset_index()
